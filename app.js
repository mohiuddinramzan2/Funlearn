// ============================================
//  ABC & 123 শিখি — Kids Learning App v2
//  Alphabet + Numbers Full Logic
// ============================================
...
let modalIndex    = 0;
let modalDataset  = ALPHABET; // which dataset modal is navigating
...
function showTab(name) {
  ...
  document.getElementById('tabSong').classList.toggle('active',  name === 'song');
...
function showNumTab(name) {
  ['num-learn','num-quiz','num-song'].forEach(t => {
    const el = document.getElementById(`tab-${t}`);
    if (el) el.classList.toggle('active', t === name);
  });
}

// Override showTab to route based on current mode
window.showTab = function(name) {
  currentTab = name;

  // update tab buttons
  ['tabLearn','tabQuiz','tabSong'].forEach(id => document.getElementById(id).classList.remove('active'));
  ...
  if (currentMode === 'alpha') {
    ...
    const map = { learn:'num-learn', quiz:'num-quiz', song:'num-song' };
    ['num-learn','num-quiz','num-song'].forEach(t => {
      const el = document.getElementById(`tab-${t}`);
      if (el) el.classList.toggle('active', t === map[name]);
    });
...
function checkAlphaAnswer(chosen, correct, btn) {
  ...
  if (chosen === correct) {
    ...
    document.querySelectorAll('#quizOptions .quiz-opt-btn').forEach(b => { if (b.textContent === correct) b.classList.add('correct'); });
...
function checkNumAnswer(chosen, correct, btn) {
  ...
  if (chosen === correct) {
    ...
    document.querySelectorAll('#numQuizOptions .quiz-opt-btn').forEach(b => { if (+b.textContent === correct) b.classList.add('correct'); });
    speakText(`The answer is ${correct}.`);
  }
...
function toggleSong() {
  songPlaying ? stopSong() : startSong();
}
function startSong() {
...
function toggleNumSong() {
  numSongPlaying ? stopNumSong() : startNumSong();
}
